...
def main():
    ...
    # support = 50
    # input_file= 'users_business_nevada.csv'
    # output_file = 'task2_output.txt'
    # Filter_Threshold = 70

    start_time = time.time()
    conf1 = (SparkConf().setMaster("local[*]")).set("spark.driver.host", "localhost")
    sc = SparkContext(appName="SONAlgorithm", conf=conf1)

    dataRDD = sc.textFile(input_file)
    # header = dataRDD_header.first()
    # dataRDD = dataRDD_header.filter(lambda string: string != header)
    ...
    basketRDD = dataRDD.reduceByKey(lambda x, y: x + ',' + y if y not in x else x). \
        map(lambda x: sorted([each for each in x[1].split(',')])).filter(lambda x: len(x) > Filter_Threshold)
    ...
    # SON phase 2
    allFrequentItemSet = CandidatesItemSet.mapPartitions(
        lambda candidates: getCountSON(list(basket), list(candidates))).filter(lambda x,: x[1] >= support).map(
